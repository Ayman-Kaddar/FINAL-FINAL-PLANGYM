              ...
              <div class="row d-flex justify-content-center  h-100">
                <div class="col col-xl-10">
                  <div class="card" style="border-radius: 1rem;">
                    <form method="POST" action="{{route('session.update')}}" id="clientStore">
                        @csrf
                        <div class="col-md-6 col-lg-5 d-flex d-md-block">
                            <div class="card-body text-black">
                                <div class="d-flex align-items-center mb-1">
                                    <span class="h1 fw-bold mb-0">Edita Entrenamiento</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card-body p-2 p-lg-5 text-black">
                                    <div class="form-outline mb-4" id="exercices">
                                        <div class="col-lg-12 mx-auto lg:mx-1 m-1 hover:underline font-bold rounded-lg lg:mt-0 py-4 px-8 shadow bg-gray-50 text-gray-800 mb-4" style="background-color: #d6c0e4;">
                                            ...
                                            <input type="hidden" name="theExercice" value="{{$training->exercice_id}}"/>
                                            <input type="hidden" name="trainingid" value="{{$training->id}}"/>
                                            <div class="form-outline m-1 mb-4">
                                                <input type="number" name="weight" class="form-control form-control-lg" value="{{$training->weight}}" min="1" max="100" required/>
                                                <label class="form-label" for="form2Example17">Peso</label>
                                            </div>
                                            <div class="form-outline m-1 mb-4">
                                                <input type="number" name="repetition" class="form-control form-control-lg" value="{{$training->repetition}}" required/>
                                                <label class="form-label" for="form2Example17">Repeticiones</label>
                                            </div>
                                            <div class="form-outline m-1 mb-4">
                                                <input type="number" name="time" class="form-control form-control-lg" value="{{$training->time}}" required/>
                                                <label class="form-label" for="form2Example17">Tiempo</label>
                                            </div>
                                            <div class="form-outline m-1 mb-4">
                                                <textarea  type="text" name="remark" class="form-control form-control-lg" value="{{$training->remark}}" placeholder="{{$training->remark}}" required></textarea>
                                                <label class="form-label" for="form2Example17">Anotación Obligatoria</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="pt-1 mb-4">
                                        <button type="submit" class="btn btn-dark btn-lg btn-block" type="button">Guardar Ejercicio</button>
                                    </div>
                                </div>
                            </div>
                        </div>
